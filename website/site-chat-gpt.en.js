...
(function () {
  ...
  var defaultPlaceholder = "Type a message…";
  ...
  function showTyping(label) {
    ...
    typingEl.setAttribute("aria-label", label || "Denis is typing");
    typingEl.innerHTML =
      '<span class="typing-label">' +
      (label || "Denis is typing") +
    ...
    slowHintTimer = setTimeout(function () {
      setTypingLabel("One moment… preparing a reply");
    }, 2500);
    slowHintTimer2 = setTimeout(function () {
      setTypingLabel("Server is waking up");
    }, 8000);
  }
  ...
  function showSetupHelp(err) {
    if (window.RB_LiteChat && typeof window.RB_LiteChat.activate === "function") {
      if (sessionId) {
        addBubble(
          "Cannot reach the server right now. Try again in a minute or message on Telegram: " +
            tgHandle,
          "bot"
        );
        afterChatTurn(false);
      ...
      return fetchApi(bases[i], path, body)
        .then(function (data) {
          ...
          if (
            ...
            i + 1 < bases.length &&
            (reply ===
              "I'm with you. Let's take this step by step. What feels hardest right now?" ||
              reply ===
                "I'm here. Briefly tell me what happened and what feels hardest right now.")
          ) {
  ...
  function armBusyWatchdog() {
    ...
    busyWatchdogTimer = setTimeout(function () {
      ...
      setBusy(false);
      addBubble(
        "The reply is taking longer than usual. Wait a moment or tap the button again.",
        "bot"
      );
      afterChatTurn(false);
    }, ms);
  ...
  clearBoot();
  if (sessionId) {
    addBubble("Continuing the conversation. Type a message or tap a button.", "bot");
  } else {
    log.innerHTML = "";
    startChat();
  }
